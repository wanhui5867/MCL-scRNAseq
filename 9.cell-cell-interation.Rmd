...
Run cell-cell communication only in tumor samples, but the cell-types could be different level as well as the analysis level  (sample levle/ overall level)
1. Malignant - TME  cell subtype 
2. Maligant subtype - TME cell subtype
3. Maligant subtype - TME cell subtype+ subcluster
...
q.all <- readRDS('Matrix/03_data_cellannot_BCRfilt.inferedMalignant.noDoublets.annotCellsubtype.correctSample.rds')
...
q.all@meta.data <- q.all@meta.data %>% 
  ...
  mutate(final_cell_type =Cell_subtype.cluster,
         final_cell_type_major = str_remove_all(final_cell_type, '\\.c\\d+'),
         final_cell_type_major.subcluster = if_else(final_cell_type_major == 'Malignant' & Donor %in% c('MC3', 'MC4', 'MC201', 'MC202', 'MC203', 'MC205', 'MC206'), Malignant.cluster, final_cell_type_major)
         ) # Need to modify

rownames(q.all@meta.data) = q.all$CellBarcode

# subset the major cell type to analysis cell talk
q.cc <- subset(q.all, subset = Cancer_type == 'MCL' & Major_cell_type %in% c('Malignant','B', 'T', 'NK', 'Myeloid'))
saveRDS(q.cc, 'Matrix/10.allpatient.cells.forCCC.rds')


# q.cc@meta.data = q.cc@meta.data %>% mutate(Tissue = if_else(Sample == 'MC3Rl', 'Intestine', Tissue))
# rownames(q.cc@meta.data) = q.cc$CellBarcode

q.cc <- readRDS('Matrix/10.allpatient.cells.forCCC.rds')
q.cc$Sample = NULL
q.cc$Sampling_time = NULL
q.cc$Tissue = NULL
q.cc$Tissue_source = NULL
q.cc$Treatment = NULL

q.cc@meta.data = q.cc@meta.data %>% left_join(phe %>% select(orig.ident, Sample, Treatment, Sampling_time, Tissue, Tissue_source))
rownames(q.cc@meta.data ) <- q.cc$CellBarcode

saveRDS(q.cc, 'Matrix/10.allpatient.cells.forCCC.correctSample.rds')
...
write.xlsx(as.data.frame(df.celltype.major), file = 'stat.cell_number.perSample.xlsx', sheetName = 'Major cell type', append = T, row.names = F)
...
write.xlsx(as.data.frame(df.celltype.subtypeT), file = 'stat.cell_number.perSample.xlsx', sheetName = 'Cell type (T+NK)', append = T, row.names = F)
...
h = 6
reduction = 'umap'
ggsave( DimPlot(q.all, reduction = reduction, group.by = "final_cell_type", repel = T, label = T, label.size = 7) + NoAxes()  +  
        ...
        filename =  paste0(mydir ,reduction, '.cell type final.T.L.png'), w = w, h = h)
...
## cell talk - not seperate samples 
...
### run liana aggreate
```{r run liana all}
liana.major <- liana_wrap(q.cc, expr_prop = 0.1, idents_col = 'final_cell_type_major' , resource =  c('OmniPath'))
liana.major %>% dplyr::glimpse()
liana.major.cp <- liana_wrap(q.cc, expr_prop = 0.1, idents_col = 'final_cell_type_major', method = 'cellphonedb' , resource =  c('OmniPath'))


# aggregate
liana.major <- liana.major %>%   liana_aggregate()
write_tsv(liana.major, file = paste0(mydir, paste0('CellTalk/CCC.majorCelltype.OmniPath.tsv')))

# filt by aggregate
liana_trunc <- liana.major %>%
  # only keep interactions concordant between methods
  filter(aggregate_rank <= 0.01) # note that these pvals are already corrected

# liana_cellph <- liana.major %>%
#   # only keep interactions concordant between methods
#   filter(cellphonedb.pvalue < 0.01) # note that these pvals are already corrected
...
```{r plot -all samples }
# filter
liana.major <- liana_wrap(q.cc, expr_prop = 0.1, idents_col = 'final_cell_type_major' , resource =  c('OmniPath')) %>%   liana_aggregate()
liana.major <- read_tsv('CellTalk/CCC.majorCelltype.tsv')
liana.major.f <- liana.major %>%  
  # filter(aggregate_rank <= 0.05) %>%   # only keep interactions concordant between methods
  filter(cellphonedb.pvalue <= 0.05) %>%   # only keep interactions concordant between methods
  inner_join(ref.lr %>% select(ligand.complex = Ligand, receptor.complex = Receptor) ) 

# state
liana.major.f %>% select(ligand.complex, receptor.complex) %>% distinct() %>% nrow() # aggrate_rank: 18 ; cellphoneDB:  32
nrow(liana.major.f) # aggrate_rank: 94 ; cellphoneDB:  1198

# stat --- 
my_dir = 'CellTalk/allSamples_MethodCellphone_OminPathDB/'
dir.create(my_dir)


# heatmap of L and R paris
df = liana.major.f %>% mutate(Celltype = source) %>% group_by(Celltype) %>% summarise(N_ligand = n()  ) %>% 
...
# defualt - 
ph <- heat_freq(liana.major.f ) 
pdf(file = paste0(my_dir, 'heatmap.L-R.pdf'), h = 8, w = 8)
ph
dev.off()

# add clusters
ph =  liana.major.f  %>% group_by(source ,target) %>% summarise(n = n()) %>% spread(source, n, fill = 0) %>% column_to_rownames('target')
...
# circle
show.cell = unique(liana.major.f$source)
chord_freq(liana.major.f, source_groups = show.cell, target_groups = show.cell)

# dotplot --- 
source.cell = 'Malignant'
target.cell = liana.major.f %>% filter(target != 'Malignant' ) %>% pull(target) %>% unique()

mydf = liana.major.f %>% filter(source %in% source.cell, target %in% target.cell) %>% mutate(Direction = 'L-R') %>% 
      mutate(connection = str_c(ligand.complex, receptor.complex, sep = '->')) %>% 
  bind_rows(liana.major.f %>% select(tmp = source, source = target, target = source, everything()) %>% 
...
ggplot(mydf ) +
  geom_point(aes(x = target, y = connection, size = -log10(cellphonedb.pvalue + 1e-10), color =sca.LRscore )) +
...
liana.major.custom = read_tsv(paste0(my_dir, 'Allsample.CCC.nofilter.tsv'))
...
### plot
```{r}
liana_trunc %>% select(ligand.complex, receptor.complex) %>% distinct() %>% nrow() #218
nrow(liana_trunc) # 
```

## Cell talk - seperated bone marrow / non-bone marrow

### filter and plot
```{r plot -BM/non-BM samples }
...
q.cc = readRDS('Matrix/10.allpatient.cells.forCCC.correctSample.rds') 
cell.filter = q.cc@meta.data %>% right_join(stat.celltype %>% select(Sample, Celltype)) %>% pull(CellBarcode)
q.cc =  subset(q.cc, subset = CellBarcode %in% cell.filter)

stat.celltype.bm = q.cc@meta.data %>% group_by(tissue, final_cell_type_major) %>% summarise(n = n() ) %>% 
  ungroup() %>% group_by(tissue) %>% mutate(total = sum(n)) %>% mutate(pct = n/total) 

q.cc$tissue = if_else(q.cc$Tissue == 'BM', 'BM', 'nonBM')
q.cc.sce = Seurat::as.SingleCellExperiment(q.cc)
mydir = 'CellTalk/BMvsNonBM/'
dir.create(mydir)
...
saveRDS(liana.major.BM, paste0(mydir, paste0('liana.', my.idents, '.rds')))
...
liana.cellph.BM <- map_df(names(liana.major.BM), .f = function(x)  liana.major.BM[[x]]$cellphonedb %>% mutate(tissue = x))
...
# stat --- 
my_dir = 'CellTalk/BMvsNonBM_MethodCellphone_OminPathDB/'
dir.create(my_dir)
...
mydf$tissue = factor(mydf$tissue, levels = c('BM', 'nonBM'))
...
# stat --- 
my_dir = 'CellTalk/pvsR_MethodCellphone_OminPathDB/'
dir.create(my_dir)
...
saveRDS(liana.major.sep, paste0(mydir, paste0('liana.sepSample', my.idents, '.rds')))
...
liana.aggre.all <- map_df(names(liana.major.sep[c(1:13,15:18)]), .f = function(x)  liana.major.sep[[x]] %>% liana_aggregate() %>% mutate(Sample = x))
write_tsv(liana.aggre.all, file = paste0(mydir, 'CCC.', my.idents,'.', 'allSamples.xls'))

# cellphoneDB & intergrate all samples
liana.cellph.all <- map_df(names(liana.major.sep[c(1:13,15:18)]), .f = function(x)  liana.major.sep[[x]]$cellphonedb %>% mutate(Sample = x))
write_tsv(liana.cellph.all, file = paste0(mydir, 'CCC.', my.idents,'.', 'allSamples.CellPhoneDB.xls'))
liana.cellph.all = read_tsv( file = paste0(mydir, 'CCC.', my.idents,'.', 'allSamples.CellPhoneDB.xls'))
...
write_tsv(stat.celltype, 'CellTalk/stat.cellpctMT1.final_cell_type_major.xls')
...
# # filt by aggregate  -- sample list
# liana_trunc <- lapply(liana.major.sep , FUN = function(x) x %>%  liana_aggregate() %>%  filter(aggregate_rank <= 0.01)  %>% 
#   inner_join(ref.lr %>% select(ligand.complex = Ligand, receptor.complex = Receptor)))
# 
# # filt by cellphone  -- sample list
# liana_cellph <- lapply(liana.major.sep , FUN = function(x) x$cellphonedb %>%  filter(pvalue < 0.01))
